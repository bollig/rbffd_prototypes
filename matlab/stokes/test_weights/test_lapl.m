...
for indx = 1:4
	...
	l2_true(i,indx) = norm(myfunc_lsfc, 2); 
        ...
        gradx = RBFFD_WEIGHTS.xsfc;
        grady = RBFFD_WEIGHTS.ysfc;
        gradz = RBFFD_WEIGHTS.zsfc;
        end
        ...
        indirect_pdx = ( RBFFD_WEIGHTS.xsfc ) * gradx;
        indirect_pdy = ( RBFFD_WEIGHTS.ysfc ) * grady;
        indirect_pdz = ( RBFFD_WEIGHTS.zsfc ) * gradz;
         
        end
        indirect_mat = indirect_pdx + indirect_pdy + indirect_pdz;
        
        indirect = indirect_mat * myfunc;
        
        indirect_err = indirect - myfunc_lsfc;
        l1_err_indirect(i) = norm(indirect_err,1)/norm(myfunc_lsfc, 1);
        l2_err_indirect(i,indx) = norm(indirect_err,2)/norm(myfunc_lsfc, 2)
...
set(gcf, 'Position', [427   430   512   240]); 
figure(7)
plotScalarfield(myfunc_lsfc, nodes, '\Delta_s[ Sph[3,2] * Sin[20x] ]')
set(gcf, 'Position', [427   430   512   240]); 
figure(8)
...
% figure(5)
% loglog(prob_sizes, conds,'o-','LineWidth',2);
