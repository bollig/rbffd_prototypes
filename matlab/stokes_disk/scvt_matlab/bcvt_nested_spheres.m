function [generators, energy, iter] = bcvt_nested_sphere(num_outer, num_inner, max_iter, nb_samples, outer_radius, inner_radius)
% Generate a constrained cvt on the boundary (nested spheres)
    ...
    % NOTE: These two lines produce clusters at poles: 
    %generators_outer = sample_sphere_boundary(outer_radius, num_outer);
    %generators_inner = sample_sphere_boundary(inner_radius, num_inner);
    
    % This is a much more random sampling: 
    generators_outer = project_to_sphere(sample_nested_spheres([inner_radius outer_radius], num_outer), num_outer, outer_radius); 
    generators_inner = project_to_sphere(sample_nested_spheres([inner_radius outer_radius], num_inner), num_inner, inner_radius); 
    ...
    all_generators = [generators_outer; generators_inner];
    ...
    for iter=0:max_iter 
        ...
        if mod(iter, 20) == 0 || iter ==0 
            scatter3(all_generators(:,1), all_generators(:,2), all_generators(:,3), 18, 'filled');
            %tes = delaunay3(all_generators(:,1), all_generators(:,2), all_generators(:,3));
            %tetramesh(tes, all_generators);
            axis square;
            pbaspect([1 1 1]);
            axis vis3d;
        ...
        %tree = kdtree_build(all_generators);
        tree = KDTreeSearcher(all_generators,'distance','euclidean');
       ...
       % scatter3(samples3D(:,1), samples3D(:,2), samples3D(:,3), 15, genIdxs, 'filled');
        ...
        % Calculate new mass centroid with random sampling
        centroids = all_generators;
        count = ones(length(all_generators), 1);
        ...
        generators = [project_to_sphere(centroids, num_outer, outer_radius); project_to_sphere(centroids, num_inner, inner_radius)];

        
        %fprintf('Draw started\n');
        %hold on; 
        all_generators = generators;
        
        %scatter3(all_generators(:,1), all_generators(:,2), all_generators(:,3), 48, 1:length(all_generators), 'filled');
        % hold off;
        fprintf('[Boundary] Iter %d Complete\n', iter);
    end
