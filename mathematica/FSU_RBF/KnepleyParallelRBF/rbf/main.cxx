...
PetscErrorCode mysubmat(Mat mat,PetscInt n,const IS irow[],const IS icol[],MatReuse scall,Mat *submat[])
{
  ...
  PetscFunctionBegin;
  if (scall == MAT_REUSE_MATRIX) {SETERRQ(((PetscObject)submat)->comm, PETSC_ERR_SUP, "Cannot handle submatrix reuse yet");}
...
//    ei = exp(-(xi*xi+yi*yi)/(4*parameter.vis*parameter.t))/(pi*4*parameter.vis*parameter.t);
  ...
  */
  ...
  ierr = ISCreateGeneral(PETSC_COMM_WORLD,particle.nlocal,isort,PETSC_USE_POINTER,&isx);CHKERRQ(ierr);
  ...
  */
  ...
  for (ic=0; ic<cluster.nclocal; ic++) {
    ...
    ierr = ISCreateGeneral(PETSC_COMM_SELF,cluster.npbufferi,idx,PETSC_USE_POINTER,&is[ic]);CHKERRQ(ierr);
    ierr = ISCreateGeneral(PETSC_COMM_SELF,iend-ista+1,idx,PETSC_USE_POINTER,&is_local[ic]);CHKERRQ(ierr);
  }
